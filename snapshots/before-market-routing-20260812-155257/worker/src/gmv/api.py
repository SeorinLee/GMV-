...
WORKER_BUILD_ID = "job-profile-clones-v10"
...
def create_app(store: JobStore | None = None, session_factory=real_session_factory) -> FastAPI:
    ...
    async def create_job(
        ...
        requested_concurrency = (
            get_default_concurrency() if concurrency is None else concurrency
        )
        job_concurrency = min(get_max_concurrency(), max(1, requested_concurrency))
        record = JobRecord(
