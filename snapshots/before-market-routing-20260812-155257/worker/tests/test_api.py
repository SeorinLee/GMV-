"""API tests for the US Chrome / UK Edge flow."""
...
def test_profiles_endpoint(tmp_path):
    async def go():
        ...
        async with client:
            ...
            assert codes == {"US_CHROME", "UK_EDGE"}
...
def test_create_job_persists_requested_and_clamped_concurrency(tmp_path, monkeypatch):
    async def go():
        monkeypatch.setenv("GMV_DEFAULT_CONCURRENCY", "5")
        monkeypatch.setenv("GMV_MAX_CONCURRENCY", "6")

        for supplied, expected in ((6, 6), (99, 6), (None, 5)):
            app, client = _make(tmp_path / str(expected) / str(supplied), {})
            async with client:
                ...
                assert record.concurrency == expected
